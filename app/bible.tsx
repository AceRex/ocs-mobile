...
export default function BibleScreen() {
  ...
  const [confirmingVerse, setConfirmingVerse] = useState<{
    index: number;
    text: string;
  } | null>(null);
  ...
  const verseList = verses.map((_, i) => ({
    index: i,
    label: (i + 1).toString(),
  }));
    ...
    <SafeAreaView className="flex-1 bg-[#121212]">
      ...
      <View className="p-3 border-b border-white/10 bg-[#1a1a1a]">
        <View className="flex-row justify-between items-center mb-3">
          ...
          >
            {isSyncEnabled ? (
              <LinkIcon size={14} color="#3b82f6" weight="bold" />
            ) : (
              <LinkBreak size={14} color="#666" />
            )}
            <Text
              className={`text-xs font-bold ${isSyncEnabled ? "text-blue-400" : "text-white/40"}`}
            >
        ...
        <View className="flex-row gap-2">
          ...
          <TouchableOpacity
            ...
            className="flex-[1.2] bg-white/5 p-2 rounded-lg border border-white/10 flex-row justify-center items-center gap-1"
          >
            <Text
          ...
          <TouchableOpacity
            ...
            className="flex-[2.5] bg-white/5 p-2 rounded-lg border border-white/10 flex-row justify-between items-center px-3"
          >
            <Text
          ...
          <TouchableOpacity
            ...
            className="flex-[1] bg-white/5 p-2 rounded-lg border border-white/10 flex-row justify-center items-center gap-1"
          >
          ...
          {/* Verse Button */}
          <TouchableOpacity
            onPress={() => setShowVersesModal(true)}
            className="flex-[0.8] bg-white/5 p-2 rounded-lg border border-white/10 flex-row justify-center items-center gap-1"
          >
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          ...
          >
            <Text className="text-white/30 font-bold w-8 pt-0.5 text-[10px]">
              {index + 1}
            </Text>
            <Text className="text-white/90 text-[16px] flex-1 leading-6">
              {item}
            </Text>
          </TouchableOpacity>
        )}
      ...
      {/* Confirmation Modal */}
      <Modal
        visible={!!confirmingVerse}
        transparent
        animationType="fade"
        onRequestClose={() => setConfirmingVerse(null)}
      >
          ...
          <View className="bg-[#1e1e1e] w-full rounded-2xl border border-white/10 p-6 shadow-2xl">
            ...
            <View className="bg-white/5 p-4 rounded-lg mb-6 max-h-40">
              <Text className="text-[#38ef7d] font-bold text-xs mb-1">
                ...
                {confirmingVerse ? confirmingVerse.index + 1 : ""}
              </Text>
              <Text className="text-white/80 italic text-sm leading-5">
                "{confirmingVerse?.text}"
              </Text>
            </View>
      ...
      <SearchableListModal
        ...
        renderItem={({ item, index }: any) => (
          ...
          >
            <Text
              className={`text-lg ${index === selectedBookIndex ? "text-[#38ef7d] font-bold" : "text-white"}`}
            >
              {item.name}
            </Text>
      ...
      >
        <FlatList
          ...
          renderItem={({ item, index }) => (
            ...
            >
              <Text
                className={`font-bold ${index === selectedChapterIndex ? "text-black" : "text-white"}`}
              >
                {item}
              </Text>
            </TouchableOpacity>
        ...
        <View className="h-[80%] bg-[#1a1a1a] rounded-t-3xl overflow-hidden flex flex-col">
          ...
          <FlatList
            ...
            contentContainerStyle={{
              paddingBottom: 40,
              paddingHorizontal: numColumns > 1 ? 10 : 0,
            }}
