...
export default function StageControlScreen() {
  ...
  const { agenda, activeId, time, isPaused } = useTimerStore();

  const isStageAuthorized = isAdmin || deviceRole === "stageManager" || deviceRole === "admin";
  const [feedback, setFeedback] = useState<{ text: string; ok: boolean } | null>(null);

  const activeAgendaItem = Array.isArray(agenda)
    ? agenda.find((item) => item._id === activeId)
    : null;
  ...
  const handleCommand = (cmd: string, label: string) => {
    if (!isStageAuthorized) {
      Alert.alert(
        "Access Denied",
        "Admin or Stage Manager privileges required for Stage Master Control."
      );
      return;
    }
  ...
  if (!isPaired || !isStageAuthorized) {
    return (
      <SafeAreaView className="flex-1 bg-zinc-950 justify-center items-center px-6">
        <StatusBar barStyle="light-content" backgroundColor="#09090b" />
        <Card className="w-full max-w-sm items-center p-6 border-zinc-800/80 bg-zinc-900/90 rounded-[12px]">
          ...
          <Text className="text-xl font-bold text-white text-center mb-2 tracking-tight">
            Stage Master Access Required
          </Text>
          <Text className="text-zinc-400 text-xs text-center leading-relaxed mb-6">
            Stage Master Control is reserved for authorized stage operators. Please pair with the OCS Desktop Controller and grant Admin or Stage Manager permission.
          </Text>
          <Button
            variant="outline"
  ...
  return (
    <SafeAreaView className="flex-1 bg-zinc-950">
      ...
      <View className="px-5 py-3 flex-row items-center justify-between border-b border-zinc-800/80 bg-zinc-950/90">
        <View className="flex-row items-center gap-3">
          <Button
            ...
            size="icon"
            onPress={() => router.back()}
            className="w-10 h-10 border-zinc-800 bg-zinc-900/80 rounded-[12px]"
          >
      ...
      >
        ...
        <Card className="mb-5 bg-zinc-900/90 border-zinc-800/80 rounded-[12px]">
          ...
          {/* Active Agenda Item Context (if available) */}
          {activeAgendaItem ? (
            <View className="mt-3 p-3 rounded-[12px] bg-zinc-950/60 border border-zinc-800/60 flex-row items-center justify-between">
              <View className="flex-1 mr-2">
                <Text className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider">
                  Service Session
                </Text>
                <Text className="text-white font-bold text-xs truncate">
                  {activeAgendaItem.agenda || "Active Session"}
                </Text>
              </View>
              {activeAgendaItem.anchor ? (
                <View className="flex-row items-center gap-1.5 px-2 py-1 rounded-[12px] bg-purple-500/10 border border-purple-500/30">
          ...
          <View className="flex-row items-center justify-between pt-3 mt-3 border-t border-zinc-800/80">
            ...
            <View className="flex-row items-center gap-2">
              <Badge variant="outline" className="rounded-[12px]">General View</Badge>
              <Badge variant="outline" className="rounded-[12px]">Speaker View</Badge>
            </View>
          </View>
        </Card>
          ...
          <CardContent className="space-y-3">
            ...
            <View className="flex-row gap-3">
              <Button
                ...
                className="flex-1 h-14 bg-zinc-900/80 border-zinc-800 rounded-[12px]"
              >
              ...
              <Button
                ...
                className="flex-1 h-14 bg-purple-600 border-purple-500 rounded-[12px]"
              >
            ...
            <View className="flex-row gap-2.5">
              <Button
                ...
                className="flex-1 py-3 bg-zinc-800/80 border border-zinc-700/50 rounded-[12px]"
              >
            ...
            {/* Return to Presentation / Teleprompter Shortcuts */}
            <View className="flex-row gap-2.5">
              <Button
                variant="outline"
                onPress={() => handleCommand("return_to_presentation", "Return to Slides")}
                className="flex-1 py-2.5 bg-zinc-950/60 border-zinc-800 rounded-[12px]"
              >
              ...
              <Button
                variant="outline"
                onPress={() => router.push("/presentation" as any)}
                className="flex-1 py-2.5 bg-zinc-950/60 border-zinc-800 rounded-[12px]"
              >
                <Monitor size={15} color="#67e8f9" weight="bold" />
                <Text className="text-cyan-300 font-bold text-xs ml-2">
                  Teleprompter
                </Text>
              </Button>
            </View>
        ...
        <Card className="mb-5 bg-zinc-900/90 border-zinc-800/80 rounded-[12px]">
          ...
          <CardContent className="space-y-3">
            <View className="flex-row gap-2">
              {["5m", "10m", "15m", "30m"].map((timeLabel) => (
                <Button
                  ...
                  className="flex-1 py-2.5 bg-zinc-900/80 border-zinc-800 rounded-[12px]"
                >
            ...
            <Button
              ...
              className="w-full py-2.5 border-red-900/40 bg-red-950/20 rounded-[12px]"
            >
        ...
        <Card className="bg-zinc-900/40 border-zinc-800/50 p-4 rounded-[12px]">
          <View className="flex-row items-center gap-3">
            ...
            <View className="flex-1">
              <Text className="text-white font-bold text-xs">Stage Master Active</Text>
